...
# Função para gerenciar API keys (produção vs desenvolvimento)
def get_api_key():
    """Obter API key de secrets (produção) ou input do usuário (desenvolvimento)"""
    # Verificar se está em produção (com secrets configurados)
    ...
    # Modo desenvolvimento - solicitar via interface
    return None

def is_production():
    """Verificar se está rodando em produção"""
    return (hasattr(st, 'secrets') and 'GOOGLE_API_KEY' in st.secrets) or ('GOOGLE_API_KEY' in os.environ)
...
def main():
    ...
    with st.sidebar:
        ...
        # API Key gerenciada automaticamente via Secrets
        api_key = get_api_key()

        if api_key:
            st.success("🔑 **Google Gemini configurado e pronto para uso!**")
        else:
            st.error("❌ **API Key não encontrada!** Verifique a configuração no Streamlit Secrets.")
            st.info("💡 Configure a GOOGLE_API_KEY nas configurações de Secrets da aplicação.")
            st.stop()



        # Botão de inicialização
        if st.button("🚀 Inicializar Agente IA", type="primary"):
            with st.spinner("Inicializando Google Gemini com Agentes..."):
